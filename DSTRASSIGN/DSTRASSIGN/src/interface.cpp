...
std::istream& operator>> (std::istream& in, Csv::severaldatatype& e)
{
...
std::ostream& operator<< (std::ostream& out, const Csv::severaldatatype& e)
{
    return out << e.to_str();
}

void readline(std::istream& in, Csv::severaldatatype& e)
{
    // flush the buffer
    in.sync();
...
namespace Interface
{
    ...
    void print_note(const severaldatatype& note,
        const severaldatatype& note_head,
        bool _clrscr,
        std::ostream& out)
    {
        if (_clrscr)
    ...
    void print_table(const table& tabl,
        const severaldatatype& tabl_head,
        bool _nocolumn,
        bool _vertical,
        bool _clrscr,
        std::ostream& out)
    {
        ...
        {
            maxl.at(0) = Max(maxl.at(0), (int)strlen(severaldatatype(i + 1).to_str()));
        ...
        if (Nrows > 0)
        {
            if (_vertical)
            {
                ...
                for (int i = 0; i < Ncols; ++i)
                {
                    severaldatatype e = tabl.get_keys().at(i);
                    out << e << "  ";
                ...
                for (int i = 0; i < Nrows; ++i)
                {
                    ...
                    for (int j = 0; j < Ncols; ++j)
                    {
                        severaldatatype e = tabl.get_row(i).at(j);
                        out << e << "  ";
            ...
            else
            {
                ...
                for (int i = 0; i < Nrows; ++i)
                {
                    ...
                    for (int j = 0; j < Ncols; ++j)
                    {
                        severaldatatype e = tabl.get_keys().at(j);
                        out << "  " << e << "  ";
    ...
    void select_menu::add_item(const int no, const severaldatatype& desc)
    {
        items.add_row();
    ...
    void select_menu::set_title(const severaldatatype& e)
    {
        title = e;
    }

    severaldatatype select_menu::print_menu_and_wait(bool _clrscr, std::ostream& out) const
    {
        if (_clrscr)
            clrscr();
    ...
    void input_menu::add_item(const severaldatatype& desc, bool line_input)
    {
    ...
    bool input_menu::remove_item_by_desc(const severaldatatype& desc, bool strict)
    {
        return items.rm_row_where("Description", desc, strict);
    }

    void input_menu::set_title(const severaldatatype& e)
    {
        title = e;
    }

    list<severaldatatype> input_menu::print_menu_and_wait(bool _clrscr, std::ostream& out) const
    {
        if (_clrscr)
            clrscr();

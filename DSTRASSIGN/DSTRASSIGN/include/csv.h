...
namespace Csv
{
    ...
    public:
        ...
        severaldatatype(const Int e)
        {
            assign(e);
        }
        severaldatatype(const Long e)
        {
            assign(e);
        }
        severaldatatype(const Char e)
        {
            assign(e);
        }
        severaldatatype(const Pchar e)
        {
            assign(e);
        }
        severaldatatype(const Double e)
        {
            assign(e);
        }
        severaldatatype(const severaldatatype& e)
        {
            assign(e);
        }
        ...
        void assign(const severaldatatype& e)
        {
        ...
        severaldatatype& operator= (const Int e)
        {
            assign(e);
            return *this;
        }
        severaldatatype& operator= (const Long e)
        {
            assign(e);
            return *this;
        }
        severaldatatype& operator= (const Char e)
        {
            assign(e);
            return *this;
        }
        severaldatatype& operator= (const Pchar e)
        {
            assign(e);
            return *this;
        }
        severaldatatype& operator= (const Double e)
        {
            assign(e);
            return *this;
        }
        severaldatatype& operator= (const severaldatatype& e)
        {
            assign(e);
            return *this;
        }
        ...
        severaldatatype& append(const severaldatatype& e)
        {
            Char* cur = to_str(),
                * app = e.to_str(),
        ...
        bool equal(const severaldatatype& e) const
        {
            if (is_type(e.type()))
        ...
        bool strict_equal(const severaldatatype& e) const
        {
            if (!is_type(e.type()))
                return false;
            if (is_type("int"))
    ...
    public:
        ...
        list(std::initializer_list<severaldatatype> e)
        {
            head = new iter();
            _size = 0;
            for (severaldatatype item : e)
                push_back(item);
        }
    ...
    private:
        ...
        {
            struct plist* next;
            list<severaldatatype> value;
        } *rows;
        list<severaldatatype> keys;
    ...
    public:
        ...
        void add_key(const severaldatatype& e)
        {
            if (has_key(e))
                return;
            keys.push_back(e);
            ...
            {
                head->next->value.push_back(severaldatatype());
                head = head->next;
            }
        }

        // entirely remove a column with name 'e' from the table.
        void rm_key(const severaldatatype& e)
        {
        ...
        Int get_key(const severaldatatype& e) const
        {
            for (Int i = 0; i < keys.size(); ++i)
                if (e.strict_equal(keys.at(i)))
                    return i;
        ...
        bool has_key(const severaldatatype& e) const
        {
            return get_key(e) != -1;
        }

        // get the name list of all table columns.
        list<severaldatatype> get_keys() const
        {
            return keys.copy();
        }

        // fix table keys down to a fixed list of keys.
        void fix_keys(list<severaldatatype> nkeys)
        {
            list<severaldatatype> curkeys = get_keys();
        ...
        void add_row()
        {
            ...
            while (head->next != nullptr)
                head = head->next;
            head->next = new plist();
            head->next->value = list<severaldatatype>(keys.size());
            ++_num_rows;
        }
        ...
        // get the 'row'-th row.
        list<severaldatatype>& get_row(Int row)
        {
            if (row < 0 || row >= _num_rows)
                return *new list<severaldatatype>();
            plist* head = rows;
            while (row-- && head->next != nullptr)
                head = head->next;
            return head->next != nullptr ? head->next->value : *new list<severaldatatype>();
        }
        list<severaldatatype> get_row(Int row) const
        {
            if (row < 0 || row >= _num_rows)
                return list<severaldatatype>();
            plist* head = rows;
            while (row-- && head->next != nullptr)
                head = head->next;
            return head->next != nullptr ? head->next->value.copy() : list<severaldatatype>();
        }
        ...
        // one is returned.
        list<severaldatatype>& get_row_where(const severaldatatype& key_search, const severaldatatype& e, bool strict = false)
        {
        ...
        list<severaldatatype> get_row_where(const severaldatatype& key_search, const severaldatatype& e, bool strict = false) const
        {
            Int key_id = get_key(key_search);
            if (key_id == -1)
                return list<severaldatatype>();
        ...
        severaldatatype& get(Int row, const severaldatatype& key)
        {
            if (row < 0 || row >= _num_rows)
                return *new severaldatatype("bad_access");
            plist* head = rows;
            while (row-- && head->next != nullptr)
                head = head->next;
            Int pos = get_key(key);
            return pos == -1 ? (*new severaldatatype("bad_access")) :
                (head->next != nullptr ? head->next->value.at(pos) : *new severaldatatype());
        }
        severaldatatype get(Int row, const severaldatatype& key) const
        {
            if (row < 0 || row >= _num_rows)
                return severaldatatype("bad_access");
            plist* head = rows;
            while (row-- && head->next != nullptr)
                head = head->next;
            Int pos = get_key(key);
            return pos == -1 ? (*new severaldatatype("bad_access")) :
                (head->next != nullptr ? head->next->value.at(pos) : severaldatatype());
        }
        ...
        // if there are multiple rows satisfying the condition, only the first valid
        // one is returned.
        severaldatatype& get_where(const severaldatatype& key_search, const severaldatatype& e, const severaldatatype& key_get, bool strict = false)
        {
        ...
        severaldatatype get_where(const severaldatatype& key_search, const severaldatatype& e, const severaldatatype& key_get, bool strict = false) const
        {
            Int key_sid = get_key(key_search),
                key_gid = get_key(key_get);
            if (key_sid == -1 || key_gid == -1)
        ...
        bool rm_row_where(const severaldatatype& key_search, const severaldatatype& e, bool strict = false)
        {
            Int key_sid = get_key(key_search);
            if (key_sid == -1)
                return false;
        ...
        table filter(bool(*_filter_func)(severaldatatype, list<severaldatatype>, table)) const
        {
    ...
    // CSV File Reader.ONLY INITIALIZE THIS ONCE in the project and
    ...
    public:
        ...
        void read_and_terminate(const Char delim = ',',const Char delim2 = '"')
        {
            if (fptr == nullptr)
                return;
            while (!csv_str.empty())
            ...
            for (Int i = _table.num_rows() - 1; i >= 0; --i)
            {
                ...
                list<severaldatatype> _row = _table.get_row(i);
